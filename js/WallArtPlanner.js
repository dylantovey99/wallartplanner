...
export default class WallArtPlanner {
    ...
    initializeElements() {
        ...
        if (this.printWidthInput) {
            this.printWidthInput.value = this.newCollection.printWidth;
        }
        if (this.printHeightInput) {
            this.printHeightInput.value = this.newCollection.printHeight;
        }
        if (this.mattWidthInput) {
            this.mattWidthInput.value = this.newCollection.mattWidth;
        }
        if (this.frameWidthSelect) {
            this.frameWidthSelect.value = '20';
        }
        if (this.frameMaterialSelect) {
            this.frameMaterialSelect.value = this.newCollection.frameMaterial;
        }
        if (this.frameCountInput) {
            this.frameCountInput.value = this.newCollection.count;
        }
    ...
    setupEventListeners() {
        ...
        if (this.wallCanvas) {
            this.wallCanvas.addEventListener('frameMove', (event) => {
                ...
                if (!frameElement.classList.contains('frame') || !event.detail) {
                    return;
                }
            ...
            this.wallCanvas.addEventListener('frameUpdate', (event) => {
                if (!event.target.classList.contains('frame') || !event.detail) {
                    return;
                }
                this.saveState();
            });
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    reader.onload = (e) => {
                        ...
                        img.onload = () => {
                            ...
                            if (this.wallHeightInput) {
                                this.wallHeightInput.value = this.wall.height.toFixed(1);
                            }
                            if (this.wallWidthInput) {
                                this.wallWidthInput.value = this.wall.width.toFixed(1);
                            }
                            if (this.wallWidthInput) {
                                this.wallWidthInput.readOnly = true;
                            }
                            if (this.wallHeightInput) {
                                this.wallHeightInput.readOnly = true;
                            }
                    ...
                    if (this.wallWidthInput) {
                        this.wallWidthInput.readOnly = false;
                    }
                    if (this.wallHeightInput) {
                        this.wallHeightInput.readOnly = false;
                    }
                    this.saveState();
                }
            });
        ...
        if (this.printWidthInput) {
            this.printWidthInput.addEventListener('input', (e) => this.newCollection.printWidth = Number(e.target.value));
        }
        if (this.printHeightInput) {
            this.printHeightInput.addEventListener('input', (e) => this.newCollection.printHeight = Number(e.target.value));
        }
        if (this.mattWidthInput) {
            this.mattWidthInput.addEventListener('input', (e) => this.newCollection.mattWidth = Number(e.target.value));
        }
        if (this.frameWidthSelect) {
            this.frameWidthSelect.addEventListener('change', (e) => this.newCollection.frameWidth = mmToInches(Number(e.target.value)));
        }
        if (this.frameMaterialSelect) {
            this.frameMaterialSelect.addEventListener('change', (e) => this.newCollection.frameMaterial = e.target.value);
        }
        if (this.frameCountInput) {
            this.frameCountInput.addEventListener('input', (e) => this.newCollection.count = Math.max(1, Math.round(Number(e.target.value))));
        }
    }
    ...
    loadSavedState() {
        ...
        try {
            ...
            const loadedWidth = state.wall && !isNaN(Number(state.wall.width)) ? Number(state.wall.width) : 0;
            const loadedHeight = state.wall && !isNaN(Number(state.wall.height)) ? Number(state.wall.height) : 0;
            ...
            if (state.gridSize && this.gridSizeSelect) {
            ...
            if (state.newCollection) {
                ...
                if (this.frameMaterialSelect) {
                    this.frameMaterialSelect.value = this.newCollection.frameMaterial || 'black';
                }
                if (this.frameWidthSelect) {
            ...
            if (this.collectionsLegend) {
                this.collectionsLegend.innerHTML = '';
            }
            this.collections = [];
            ...
            if (state.collections) {
                state.collections.forEach(collectionData => {
                    ...
                    if (this.collectionsLegend) {
                        this.collectionsLegend.appendChild(collection.element);
                    }
        ...
        this.collections.forEach(collection => {
            collection.frames.forEach(frame => {
                if (frame.dragManager) {
                    frame.dragManager.setMinDistance(this.frameSpacing);
                }
            });
        });
    }
    ...
    updateWallDisplay() {
        if (!this.wallCanvas || !this.wallWidthDisplay || !this.wallHeightDisplay) {
            return;
        }

        const targetPixelWidth = this.wall.width * SCALE;
        const targetPixelHeight = this.wall.height * SCALE;
